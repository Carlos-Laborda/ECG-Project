...
def benchmark_dataloader_workers(dataset, batch_size=64, num_epochs=2):
    """
    Benchmark DataLoader performance with different numbers of workers.
    ...
    Returns:
        dict: Dictionary mapping num_workers to loading time
    """
    ...
    for num_workers in range(0, mp.cpu_count() + 1, 2):  # Include 0 for synchronous loading
        loader = DataLoader(
            dataset,
            shuffle=True,
        ...
        for epoch in range(num_epochs):
            for i, _ in enumerate(loader, 0):
                pass
        end = time()
        
        total_time = end - start
        results[num_workers] = total_time
        print(f"Time taken: {total_time:.2f}s with num_workers={num_workers}")
