...
# # --- Plot for SSL Models with Linear Classifier ---
...
# df_ssl_linear['model'] = pd.Categorical(
#     df_ssl_linear['model'], 
#     categories=ssl_models,
#     ordered=True
# )
# # Sort the dataframe to ensure order
# df_ssl_linear = df_ssl_linear.sort_values('model')
# models_ssl_linear = ssl_models
# n_models_ssl_linear = len(models_ssl_linear)
# x_ssl_linear = np.arange(len(fractions_ssl_linear)) * 1.5

# # Plot
# fig_ssl_linear, ax_ssl_linear = plt.subplots(figsize=(14, 7), dpi=300)

# # Plot each SSL model (Linear)
# for i, model in enumerate(models_ssl_linear):
#     sub = df_ssl_linear[df_ssl_linear["model"] == model].sort_values("label_fraction")
#     means = sub["mean"].values
#     yerr_lower = means - sub["ci_lower"].values
#     yerr_upper = sub["ci_upper"].values - means

#     positions = x_ssl_linear + i * bar_width
...
#     for j, (m1, m2) in enumerate(comparisons):
#         if comparisons[(m1, m2)] and m1 in models_ssl_linear and m2 in models_ssl_linear:
#             i1 = models_ssl_linear.index(m1)
#             i2 = models_ssl_linear.index(m2)
#             x1 = base_x + i1 * bar_width
#             x2 = base_x + i2 * bar_width
#             y = height + j * 3
...
# # Styling for SSL Linear
# ax_ssl_linear.set_title("SSL Models with Linear Classifier", fontsize=14, fontweight='bold', pad=20)
# ax_ssl_linear.set_xlabel("Proportion of Labeled Training Data", fontsize=12, fontweight='bold')
# ax_ssl_linear.set_ylabel("F1 Score", fontsize=12, fontweight='bold')
# ax_ssl_linear.set_xticks(x_ssl_linear + bar_width * (n_models_ssl_linear - 1) / 2)
# ax_ssl_linear.set_xticklabels([f'{int(f*100)}%' for f in fractions_ssl_linear], fontsize=12)
# ax_ssl_linear.tick_params(axis='both', which='major', labelsize=12) 
# ax_ssl_linear.grid(True, linestyle='--', alpha=0.3)
# ax_ssl_linear.set_facecolor('#FAFAFA')
# ax_ssl_linear.spines['top'].set_visible(False)
# ax_ssl_linear.spines['right'].set_visible(False)
# ax_ssl_linear.spines['left'].set_linewidth(0.5)
# ax_ssl_linear.spines['bottom'].set_linewidth(0.5)
# ax_ssl_linear.legend(frameon=True, fancybox=True, shadow=True, fontsize=11, loc='lower right')

# plt.tight_layout()
# plt.show()


# # --- Plot for SSL Models with MLP Classifier ---
...
# # Create a categorical type with specific order
# df_ssl_mlp['model'] = pd.Categorical(
#     df_ssl_mlp['model'], 
#     categories=ssl_models,
#     ordered=True
# )
...
# models_ssl_mlp = ssl_models
# n_models_ssl_mlp = len(models_ssl_mlp)
# x_ssl_mlp = np.arange(len(fractions_ssl_mlp)) * 1.5

# # Plot
# fig_ssl_mlp, ax_ssl_mlp = plt.subplots(figsize=(14, 7), dpi=300)

# # Plot each SSL model (MLP)
# for i, model in enumerate(models_ssl_mlp):
#     sub = df_ssl_mlp[df_ssl_mlp["model"] == model].sort_values("label_fraction")
#     means = sub["mean"].values
#     yerr_lower = means - sub["ci_lower"].values
#     yerr_upper = sub["ci_upper"].values - means

#     positions = x_ssl_mlp + i * bar_width
#     ax_ssl_mlp.errorbar(
#         positions, means, yerr=[yerr_lower, yerr_upper],
#         fmt=MARKERS.get(model, 'o'),
#         capsize=5,
...
# # Add significance asterisks for SSL MLP
# for idx, frac in enumerate(fractions_ssl_mlp):
#     comparisons = SSL_MLP_SIGNIFICANCE.get(frac, {})
#     base_x = x_ssl_mlp[idx]
...
#     for j, (m1, m2) in enumerate(comparisons):
#         if comparisons[(m1, m2)] and m1 in models_ssl_mlp and m2 in models_ssl_mlp:
#             i1 = models_ssl_mlp.index(m1)
#             i2 = models_ssl_mlp.index(m2)
#             x1 = base_x + i1 * bar_width
#             x2 = base_x + i2 * bar_width
#             y = height + j * 3
...
# # Styling for SSL MLP
# ax_ssl_mlp.set_title("SSL Models with MLP Classifier", fontsize=14, fontweight='bold', pad=20)
# ax_ssl_mlp.set_xlabel("Proportion of Labeled Training Data", fontsize=12, fontweight='bold')
# ax_ssl_mlp.set_ylabel("F1 Score", fontsize=12, fontweight='bold')
# ax_ssl_mlp.set_xticks(x_ssl_mlp + bar_width * (n_models_ssl_mlp - 1) / 2)
# ax_ssl_mlp.set_xticklabels([f'{int(f*100)}%' for f in fractions_ssl_mlp], fontsize=12)
# ax_ssl_mlp.tick_params(axis='both', which='major', labelsize=12) 
# ax_ssl_mlp.grid(True, linestyle='--', alpha=0.3)
# ax_ssl_mlp.set_facecolor('#FAFAFA')
# ax_ssl_mlp.spines['top'].set_visible(False)
# ax_ssl_mlp.spines['right'].set_visible(False)
# ax_ssl_mlp.spines['left'].set_linewidth(0.5)
# ax_ssl_mlp.spines['bottom'].set_linewidth(0.5)
# ax_ssl_mlp.legend(frameon=True, fancybox=True, shadow=False, fontsize=11, loc='lower right')
...
fig_ssl, (ax_ssl_linear, ax_ssl_mlp) = plt.subplots(1, 2, figsize=(20, 7), dpi=300)
