...
from tstcc_soft import data_generator_from_arrays, Trainer_wo_DTW, tstcc_soft, TC, Config as ECGConfig, \
    LinearClassifier, train_linear_classifier, evaluate_classifier, encode_representations, show_shape
...
class ECGTSTCCFlow(FlowSpec):
    ...
    def train_tstcc(self):
        print(f"Training TS-TCC soft version on {self.device}")
        ...
        # MLflow params
            ...
            # save checkpoint
            ckpt = os.path.join(run_dir, f"tstcc_soft_{self.mlflow_run_id}.pt")
            torch.save({
